<template>
  <div class="app-container">
    ...
    <el-card shadow="never">
      <div class="mb-10px">
        <el-button
          v-hasPerm="['release:pmsReleasePlan:add']"
          type="success"
        ...
        <el-button
          v-hasPerm="['release:pmsReleasePlan:delete']"
          type="danger"
      ...
      >
        ...
        <el-table-column fixed="right" label="操作" width="220">
          <template #default="scope">
            <el-button
              v-hasPerm="['release:pmsReleasePlan:edit']"
              type="primary"
              size="small"
              link
...
import PmsReleasePlanAPI, {
  PmsReleasePlanPageVO,
  PmsReleasePlanForm,
  PmsReleasePlanPageQuery,
} from "@/api/release/pms-release-plan";
...
const queryParams = reactive<PmsReleasePlanPageQuery>({
  pageNum: 1,
  pageSize: 10,
});

// 发布计划表格数据
const pageData = ref<PmsReleasePlanPageVO[]>([]);
...
// 发布计划表单数据
const formData = reactive<PmsReleasePlanForm>({});
...
function handleQuery() {
  loading.value = true;
  PmsReleasePlanAPI.getPage(queryParams)
    .then((data) => {
...
function handleOpenDialog(id?: number) {
  ...
  if (id) {
    dialog.title = "修改发布计划";
    PmsReleasePlanAPI.getFormData(id).then((data) => {
...
function handleSubmit() {
  dataFormRef.value.validate((valid: any) => {
    if (valid) {
      ...
      if (id) {
        PmsReleasePlanAPI.update(id, formData)
          .then(() => {
        ...
        PmsReleasePlanAPI.add(formData)
          .then(() => {
...
function handleDelete(id?: number) {
  ...
  ElMessageBox.confirm("确认删除已选中的数据项?", "警告", {
    ...
    () => {
      loading.value = true;
      PmsReleasePlanAPI.deleteByIds(ids)
        .then(() => {
